...
1. **Middle mouse / Space** → `beginPan(...)`
2. **Resize handle** → select the node, then `beginResize(nodeId, handle, ...)` after the drag threshold
3. **Node** → select the node, then `beginNodeDrag(nodeId, ...)` after the drag threshold
4. **Empty canvas** → clear selection, then `beginBoxSelect(...)` after the drag threshold
