...
This example uses `addCommandGuard` to register a command guard. The board still renders and the camera still moves, but mutation commands exit early when read-only mode is enabled.
...
const stopReadOnly = engine.addCommandGuard((name, _args, next) => {
