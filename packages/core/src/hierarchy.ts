...
import type { Bounds, CanvasNode, NodeId, Point } from './types'

export function getBoundsFromNode(node: Pick<CanvasNode, 'x' | 'y' | 'width' | 'height'>): Bounds {
  return {
    minX: node.x,
    minY: node.y,
...
export function groupArea(node: CanvasNode): number {
  return node.width * node.height
}

/** Add every descendant of `rootId` into `out` (recursive; includes nested groups). */
export function addDescendants(rootId: NodeId, nodes: Map<NodeId, CanvasNode>, out: Set<NodeId>): void {
...
export function expandGroupDragSeeds(seedIds: Iterable<NodeId>, nodes: Map<NodeId, CanvasNode>): Set<NodeId> {
...
export function collectSubtreeIds(rootId: NodeId, nodes: Map<NodeId, CanvasNode>, into: Set<NodeId>): void {
  into.add(rootId)
  for (const n of nodes.values()) {
 ...
 */
export function collectUniformTranslationTargets(seedIds: Iterable<NodeId>, nodes: Map<NodeId, CanvasNode>): NodeId[] {
...
export function findContainingGroup(node: CanvasNode, nodes: Map<NodeId, CanvasNode>): NodeId | undefined {
...
export function sortIdsByZIndex(ids: NodeId[], nodes: Map<NodeId, CanvasNode>): NodeId[] {
  return [...ids].sort((a, b) => {
