...
import { createTouchSession, type TouchIntent } from './touch-session.js'
import {
  findBoardNodeId,
  findBoardResizeHandle,
...
export function usePointerInteraction(options: UsePointerInteractionOptions) {
  ...
  function queuePointerUpdate(
    pointerId: number,
    point: Point,
    shift: boolean,
    space: boolean,
  ): void {
    pendingPointer = { id: pointerId, point, shift, space }
  ...
  function prepareTouchIntent(
    nodeId?: string,
    handle?: ResizeHandle,
  ): TouchIntent | null {
    if (handle && nodeId) {
      if (!tryBoardCommand(() => engine.select(asNodeId(nodeId)))) return null
      return { kind: 'resize', nodeId, handle }
    }
    if (nodeId) {
      const id = asNodeId(nodeId)
      if (
        !engine.getSelection().includes(id) &&
        !tryBoardCommand(() => engine.select(id))
      ) {
        return null
      }
      return { kind: 'drag', nodeId }
    }
    return { kind: 'pan' }
  }

  const touchSession = createTouchSession({
    dragThreshold: POINTER_DRAG_THRESHOLD,
    start(intent, pointerId, startPoint) {
      if (intent.kind === 'pan') {
        return startPointerInteraction(pointerId, startPoint, 'pan')
      }
      if (intent.kind === 'drag') {
        return startPointerInteraction(
          pointerId,
          startPoint,
          'drag',
          intent.nodeId,
    ...
    update: (pointerId, point) =>
      queuePointerUpdate(pointerId, point, false, false),
    flush: flushPendingPointer,
    cancelQueued: cancelPendingPointer,
    end: (pointerId) =>
      runBoardCommand(() => interaction.endInteraction(pointerId)),
    cancel: (pointerId) =>
      runBoardCommand(() => interaction.cancelInteraction(pointerId)),
    clearSelection: () => runBoardCommand(() => engine.clearSelection()),
    updatePinch(midpoint, pan, zoomDelta) {
      runBoardCommand(() =>
        engine.batch(() => {
          engine.panBy(pan.x, pan.y)
          engine.zoomAt(midpoint, zoomDelta)
        }),
      )
    },
  })
  ...
  function onPointerDown(event: PointerEvent): void {
    ...
    if (event.pointerType === 'touch') {
      ...
      rootElement.value?.focus()
      const intent =
        touchSession.getState().kind === 'idle'
          ? prepareTouchIntent(
              findBoardNodeId(event.target),
              findBoardResizeHandle(event.target),
            )
          : { kind: 'pan' as const }
      if (intent) touchSession.pointerDown(event.pointerId, localPoint, intent)
      return
    }
  ...
  function onPointerMove(event: PointerEvent): void {
    ...
    const localPoint = toLocalPoint(event.clientX, event.clientY)
    if (event.pointerType === 'touch') {
      touchSession.pointerMove(event.pointerId, localPoint)
      return
    }
  ...
  function onPointerUp(event: PointerEvent): void {
    ...
    if (event.pointerType === 'touch') {
      touchSession.pointerUp(event.pointerId)
      return
    }
  ...
  function onPointerCancel(event: PointerEvent): void {
    ...
    if (event.pointerType === 'touch') {
      touchSession.pointerCancel(event.pointerId)
  ...
  function onDoubleClick(event: MouseEvent): void {
    ...
    const nodeId =
      findBoardNodeId(event.target) ?? findNodeIdAtPoint(screenPoint)
    if (nodeId) {
      const id = asNodeId(nodeId)
