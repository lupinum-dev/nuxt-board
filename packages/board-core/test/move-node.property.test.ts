...
const moveDelta = fc.record({ dx: finiteInt(-200, 200), dy: finiteInt(-200, 200) })
...
const buildEngineWithNodes = (count: number) => {
  ...
  for (let i = 0; i < count; i++) {
    ids.push(engine.createNode({ type: 'text', x: i * 50, y: i * 30, data: {} }))
  }
  return { engine, ids }
}
...
describe('moveNode invariants (refactor pin)', () => {
  it('preserves total node count under any move sequence', () => {
    fc.assert(
      fc.property(fc.array(moveDelta, { minLength: 1, maxLength: 30 }), (deltas) => {
  ...
  it('moving a node never affects other nodes (no group, no parent)', () => {
    fc.assert(
      fc.property(fc.array(moveDelta, { minLength: 1, maxLength: 20 }), (deltas) => {
        const { engine, ids } = buildEngineWithNodes(3)
        const others = [ids[1]!, ids[2]!].map((n) => ({ id: n.id, x: n.x, y: n.y }))
        for (const { dx, dy } of deltas) engine.moveNode(ids[0]!.id, dx, dy)
  ...
  it('moveNode emits exactly one node:moved event per call', () => {
    fc.assert(
      fc.property(fc.array(moveDelta, { minLength: 1, maxLength: 10 }), (deltas) => {
        const { engine, ids } = buildEngineWithNodes(1)
        let emitted = 0
