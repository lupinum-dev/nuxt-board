...
function seed() {
  engine.importJSON(JSON.stringify({
    ...
    nodes: [
      { id: 'root', type: 'topic', x: 300, y: 160, width: 220, height: 110, data: { title: 'Product roadmap', detail: 'Q3 priorities', depth: 0 }, zIndex: 1, locked: false, visible: true },
      { id: 'eng', type: 'topic', x: 40, y: 40, width: 200, height: 100, data: { title: 'Engineering', detail: 'Performance + infra', depth: 1 }, parentId: 'root', zIndex: 2, locked: false, visible: true },
      { id: 'design', type: 'topic', x: 40, y: 290, width: 200, height: 100, data: { title: 'Design', detail: 'Design system v2', depth: 1 }, parentId: 'root', zIndex: 3, locked: false, visible: true },
      { id: 'growth', type: 'topic', x: 610, y: 40, width: 200, height: 100, data: { title: 'Growth', detail: 'Activation funnels', depth: 1 }, parentId: 'root', zIndex: 4, locked: false, visible: true },
      { id: 'api', type: 'topic', x: 610, y: 290, width: 200, height: 90, data: { title: 'API v3', depth: 2 }, parentId: 'eng', zIndex: 5, locked: false, visible: true }
    ],
    selection: [],
...
function addBranch() {
  ...
  const newNode = engine.createNode({
    ...
    width: 200,
    height: 90,
    data: { title: `New topic ${branchCount}`, depth: Math.min(parentDepth + 1, 2) },
    parentId: parent,
    select: true
  })
...
onMounted(async () => {
  engine.on('selection:change', (next) => { selection.value = next })
  seed()
...
<template>
  <div class="demo-frame">
    <div class="demo-toolbar">
      ...
      <button class="demo-primary" @click="addBranch">
        Add branch
      </button>
      ...
      <span class="demo-toolbar-note ml-auto">Select a topic, then add a branch from it.</span>
    </div>
