...
function push(message: string) {
  entries.value = [`${new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' })}  ${message}`, ...entries.value].slice(0, 18)
}
...
onMounted(() => {
  unsubscribes.push(
    ...
    engine.on('selection:change', (selected) => push(`selection:change [${selected.join(', ') || 'none'}]`)),
    engine.on('interaction:start', (state) => push(`interaction:start ${state.mode}`)),
    engine.on('interaction:end', (state) => push(`interaction:end ${state.mode}`)),
