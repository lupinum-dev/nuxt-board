...
const engine = createBoardEngine({
  initialNodes: [
    { id: asNodeId('welcome'), type: 'text', x: 100, y: 100, data: { content: 'Welcome' } }
  ]
})
```
:::
...
::tip
If the scene is bigger, build a deterministic snapshot once and import that snapshot on both sides.
::
