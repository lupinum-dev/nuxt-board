...
```
```bash [npm]
npm install @lupinum/board-minimap
```
::
...
```ts
function useMinimap(engine: BoardEngine, options?: MinimapOptions): {
...
```ts
import { useMinimap } from '@lupinum/board-minimap'
...
- [Connections & Minimap example](/examples/connections-and-minimap) for a live demo
- [Camera guide](/guides/essentials/camera-and-navigation) for viewport navigation
