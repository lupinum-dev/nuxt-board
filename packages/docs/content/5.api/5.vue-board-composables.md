...
| `snapshot` | `ShallowRef<BoardSnapshot>` | Reactive board snapshot (updated after each command). |
...
const visible = useVisibleNodes(300) // 300px margin
</script>
...
<template>
  <div v-if="bounds" class="custom-box-select" :style="{
    left: bounds.minX + 'px',
    top: bounds.minY + 'px',
