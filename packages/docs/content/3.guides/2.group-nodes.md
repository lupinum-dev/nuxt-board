...
const card = engine.createNode({
  ...
  height: 140,
  parentId: group.id,
  data: { content: 'Write release notes' },
})
...
Custom renderers receive the same node record as other node types. Read group title from `node.data.title` and color from `node.color`.
...
const props = defineProps<{
  node: BoardNode<'group', { title?: string }>
  selected: boolean
}>()
</script>
...
<template>
  <div class="group-shell" :class="{ 'is-selected': selected }">
    <span>{{ props.node.data.title ?? 'Untitled group' }}</span>
  </div>
</template>
```
