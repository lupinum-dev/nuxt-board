...
UI-created edges use automatic endpoints by default. The edge stores no `fromAnchor` or `toAnchor`, so the connection layer keeps choosing the best side as nodes move. Set `endpointMode="manual"` on `BoardConnectionLayer`, or `connectionPlugin({ endpointMode: 'manual' })`, when drag-created edges should lock to the side handles the user chose.
