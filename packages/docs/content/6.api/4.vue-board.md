...
| `simple` | Screen size 8–120px               | Minimal placeholder that preserves color and group shape. |
...
| `Space + drag/resize`                   | Temporarily bypass snapping while the pointer is active. |
...
| Action            | Behavior                                                                      |
| ----------------- | ----------------------------------------------------------------------------- |
| Remove            | Deletes the selected nodes.                                                   |
| Set colour        | Opens the preset palette and applies the chosen color to every selected node. |
| Zoom to selection | Fits the selected nodes in the viewport.                                      |
| Edit              | Starts text editing for a single selected node.                               |

Palette options use the same `BoardColorPreset` IDs as `BoardNode.color`.
