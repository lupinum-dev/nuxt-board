...
if (probe.size !== 2) {
  throw new Error(
    'Action verification must inspect flow-style steps and reusable workflows.',
  )
}
...
for (const path of await workflowFiles('.github/workflows')) {
  ...
  for (const reference of actionReferences(workflow, path))
    references.add(reference)
}
if (references.size === 0)
  throw new Error('No pinned action references were found.')
