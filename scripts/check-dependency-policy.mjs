...
export function checkDependencyPolicy(source, now = Date.now()) {
  ...
  if (!isMap(document.contents))
    return ['pnpm-workspace.yaml must contain a mapping.']
  const failures = []
  ...
  if (!isSeq(exclusions))
    return [...failures, 'minimumReleaseAgeExclude must be a list.']
  const seen = new Set()
  for (const item of exclusions.items) {
    ...
    if (
      typeof name !== 'string' ||
      !new RegExp(`^(?:@[a-z0-9._-]+/)?[a-z0-9._-]+@${version}$`).test(name)
    ) {
      failures.push(
    ...
    if (seen.has(name))
      failures.push(`${name}: duplicate quarantine exclusion.`)
    seen.add(name)
    let metadata
    try {
      metadata = JSON.parse(item.comment ?? '')
    } catch {
      /* Report missing or malformed metadata below. */
    }
    if (
      !metadata ||
      ...
      failures.push(
        `${name}: inline JSON comment must contain a nonempty reason, owner, and expires.`,
      )
      continue
    }
    ...
    if (
      ...
      new Date(timestamp).toISOString() !== expires.replace('Z', '.000Z')
    ) {
      failures.push(
        `${name}: expires must be a valid UTC timestamp (YYYY-MM-DDTHH:mm:ssZ).`,
      )
    } else if (timestamp <= now) {
      failures.push(
        `${name}: quarantine exception expired at ${expires}; remove the exclusion and its comment.`,
      )
    } else if (timestamp > now + 24 * 60 * 60 * 1000) {
      failures.push(
        `${name}: quarantine exception must expire within 24 hours.`,
      )
    }
  }
  return failures
...
if (
  ...
  if (process.argv.length > 3)
    throw new Error(
      'Usage: node scripts/check-dependency-policy.mjs [pnpm-workspace.yaml]',
    )
  const path =
    process.argv[2] ?? new URL('../pnpm-workspace.yaml', import.meta.url)
