...
if (initialStatus) {
  throw new Error(
    `Package preview requires a clean worktree:\n${initialStatus}`,
  )
}
...
if (packages.length !== 5) {
  throw new Error(`Expected five package previews, found ${packages.length}.`)
}

const manifestPath = resolve(outputDirectory, 'preview-manifest.json')
writeFileSync(
  manifestPath,
  `${JSON.stringify({ sourceSha, packages }, null, 2)}\n`,
)
