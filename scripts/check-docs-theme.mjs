...
for (const [name, pattern] of [
  ...
  requireMatch(
    appConfig,
    pattern,
    `app.config.ts must set ${name} to the Nuxt theme value.`,
  )
}
...
if (docsPackage.dependencies?.['@lupinum/ginko-content'] !== '1.0.0-beta.5') {
  failures.push(
    'docs/package.json must use @lupinum/ginko-content 1.0.0-beta.5.',
  )
}
if (packageSource.includes('pkg.pr.new')) {
  failures.push(
    'docs/package.json must not use an ephemeral pkg.pr.new dependency.',
  )
}
...
for (const token of [
  ...
  requireMatch(
    themeCss,
    new RegExp(token),
    `theme.css must define ${token.slice(0, -1)}.`,
  )
}
