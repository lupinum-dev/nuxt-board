...
if (
  manifest.sourceSha !== sourceCommit ||
  process.env.PREVIEW_SHA !== sourceCommit
) {
  throw new Error('The preview commit does not match the pull request commit.')
}

const returnedUrls = new Set(
  process.env.PREVIEW_URLS?.match(/https:\/\/\S+/gu) ?? [],
)
...
process.stdout.write(
  `Verified ${manifest.packages.length} commit-addressed package previews.\n`,
)
