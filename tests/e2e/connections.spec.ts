...
test('keeps connections attached through drag resize and zoom interactions', async ({ page }) => {
  ...
  await output.click()
  const handle = page.locator('[data-node-id="output"] [data-resize="se"]')
  ...
  await page.mouse.down()
  await page.mouse.move(handleBox.x + handleBox.width / 2 + 50, handleBox.y + handleBox.height / 2 + 35, { steps: 10 })
  await page.mouse.up()
