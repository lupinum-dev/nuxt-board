...
test('renders the new decision-first landing page', async ({ page }) => {
  const errors = await openDocs(page)
  await expect(
    page.getByRole('heading', { name: /build the board your product needs/i }),
  ).toBeVisible()
  await expect(
    page.getByRole('heading', { name: 'Command inspector' }),
  ).toBeVisible()
...
test('navigates through solutions and reference', async ({ page }) => {
  const errors = await openDocs(page, '/solutions/planning-board')
  await expect(
    page.getByRole('heading', { name: 'Planning Board' }),
  ).toBeVisible()
  await expect(page.locator('.board-root').first()).toBeVisible()

  await openDocs(page, '/reference/board-core')
  await expect(
...
test('exposes command state and publication order', async ({ page }) => {
  const errors = await openDocs(page, '/evaluate/how-vue-board-works')
...
test('keeps raw Markdown actions aligned after client navigation', async ({
  page,
}) => {
  const errors = await openDocs(page)
  await page.getByRole('link', { name: 'See how Vue Board works' }).click()
  await expect(page).toHaveURL(/\/evaluate\/how-vue-board-works$/)
  ...
  const markdownHref = await page
    ...
    .getAttribute('href')
  expect(markdownHref).toBe('/raw/evaluate/how-vue-board-works.md')
  expect(errors).toEqual([])
})
...
test('stacks lab inspectors below the stage on mobile', async ({ page }) => {
  ...
  const errors = await openDocs(
    page,
    '/understand-the-system/document-and-session-state',
  )
